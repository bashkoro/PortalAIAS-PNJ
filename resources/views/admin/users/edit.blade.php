...
@section('content')
    <div class="max-w-3xl mx-auto">
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-extrabold text-gray-900 tracking-tight">Edit Pengguna</h2>
                <p class="text-sm text-gray-500 mt-1">Perbarui data atau kata sandi pengguna.</p>
            </div>
            ...
            <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
                ...
                <div class="p-8 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Password -->
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Kata Sandi Baru <span class="text-gray-400 font-normal text-xs">(Kosongkan jika tidak diubah)</span></label>
                        ...
                        <div>
                            ...
                            <select name="hak_akses_id" required class="block w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors bg-gray-50 focus:bg-white text-sm appearance-none">
                                ...
                                @foreach($hakAkses as $role)
                                    <option value="{{ $role->id }}" {{ old('hak_akses_id', $user->hak_akses_id) == $role->id ? 'selected' : '' }}>
                                        {{ $role->nama_hak_akses }}
                                    </option>
                                @endforeach
                            </select>
