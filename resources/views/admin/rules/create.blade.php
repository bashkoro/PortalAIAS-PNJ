...
@section('title', 'Buat Aturan AIAS')

@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-extrabold text-gray-900 tracking-tight">Buat Aturan Baru</h2>
                <p class="text-sm text-gray-500 mt-1">Tambahkan aturan baru (IF-THEN) ke dalam basis pengetahuan (Knowledge Base).</p>
            </div>
                ...
                <div class="p-8">
                            ...
                            <div>
                                ...
                                <select name="kondisi[{{ $key }}]" class="block w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors bg-gray-50 focus:bg-white text-sm appearance-none">
                                    ...
                                    @foreach($options as $option)
                                        <option value="{{ $option }}" {{ old('kondisi.'.$key) == $option ? 'selected' : '' }}>
                                            {{ $option }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="tingkat_aias_id" required class="block w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors bg-gray-50 focus:bg-white text-sm appearance-none">
                            <option value="" disabled selected>-- Pilih Tingkat AIAS --</option>
                            @foreach($levels as $level)
                                <option value="{{ $level->id }}" {{ old('tingkat_aias_id') == $level->id ? 'selected' : '' }}>
                                    {{ $level->nama_tingkat }}
                                </option>
                            @endforeach
                        </select>
